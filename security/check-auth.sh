#!/bin/bash
# Check if user is authorized to trigger PR reviews
# Only organization OWNER and MEMBER roles are allowed
...
if echo "$ALLOWED_ROLES" | jq -e --arg assoc "$ASSOCIATION" '. | any(. == $assoc)' > /dev/null 2>&1; then
  ...
  echo "::error::"
  echo "::error::Only organization owners and members can trigger reviews."
  echo "::error::External contributors cannot use this action."
  echo "::error::"
  echo "::error::If you are a maintainer, ensure you have OWNER or MEMBER"
  echo "::error::role in the organization."
  echo "::error::═══════════════════════════════════════════════════════"
