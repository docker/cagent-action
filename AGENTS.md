...
| `rename-consumers.yml`            | One-shot rename migration: opens PRs across consumer repos rewriting `docker/cagent-action` refs to `docker/docker-agent-action` (dry-run by default, `repos` allowlist for pilots). |
